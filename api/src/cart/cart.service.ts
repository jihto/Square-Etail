...
export class CartService {
    ...
    async userCart(
        ...
        try {
            ...
            const query: string[] = await cartUser.products.map((item: ProductInTheCartDto)=>  item.product._id.toString() );  
            const dataProducts: any = await this.getProductsFromDjango(query); //call get data from Django 
            if(!dataProducts || typeof dataProducts === "string")
                throw new HttpException("Can't get data products", HttpStatus.BAD_GATEWAY); 
            for(let i = 0; i < cartUser.products.length; i++){ 
                const index: number = dataProducts.findIndex((item) => item.id == cartUser.products[i].product._id);
                if(index !== -1){
                    const { size, ...prev } = dataProducts[index];
                    cartUser.products[i].product = {
                        size: cartUser.products[i].product.size,
                        ...prev
                    };
                    cartUser.totalPrice += parseFloat(dataProducts[index].price) * cartUser.products[i].count;
                    cartUser.quantity += cartUser.products[i].count;
                };
            }  
    ...
    async updateAll(
        ...
        try { 
            ...
            if(!cartData) throw new NotFoundException(`Not found cart ID`);     
            cartData.products = products.map((item: any) => ({ 
                product: { 
            ...
            return new HttpException("Update  all data of Cart Success", HttpStatus.OK);
        } catch (error) {
            throw new HttpException(error.message, HttpStatus.BAD_GATEWAY)
        }
    }
